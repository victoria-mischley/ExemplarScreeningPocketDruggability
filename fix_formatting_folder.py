...
if __name__ == '__main__':
    args = args()
    working_directory = Path(args.folder_path)
    files = [file for file in working_directory.glob('*') if file.is_file()]
    for file in files:
        if file != "/Users/mischlv/Downloads/TopStructuresForExemplars/.DS_Store":
            print(file)
            AF, AF_chainA, AF_chainB, new_atom_num_json_num = main(file, args.protein1_chains_input, args.protein2_chains_input)
            file_path = Path(file)
